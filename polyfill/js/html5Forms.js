var html5Forms = new function () {
	...
	var scriptNode = null,
		scriptDir = '/secure/travel/pollyfill/js',
	...
	me.init = function () {
		...
		for (var i=0; i<scriptNodes.length; i++) {
			...
			if (scriptNode.src.match('pollyfill(_src|-p|)\.js$')) {
					...
					switch(supportReq) {
								...
								if (!Modernizr.input.required || hasBadValidationImplementation || me.forceJSValidation) {
									...
									if (isScriptCompressed) {
										toLoad = toLoad.concat([  
											scriptDir + '../../js/weston.ruter.net/webforms2/webforms2-p.js']);
									} else {
										toLoad = toLoad.concat([  
											scriptDir + '../../js/weston.ruter.net/webforms2/webforms2_src.js']);
									}
						...
						case "color":
							if (!inputSupport.color) {
								...
								toLoad = toLoad.concat([  scriptDir + '../../js/jscolor/jscolor.js']);
						...
						case "date":
							...
							if (!inputSupport.date || me.forceJSDatePicker) {
								toLoad = toLoad.concat([  
										  scriptDir + '../../js/jscalendar-1.0/calendar-win2k-1.css',
										  scriptDir + '../../js/jscalendar-1.0/calendar.js', 
										  scriptDir + '../../js/jscalendar-1.0/lang/calendar-' + lang + '.js', 
										  scriptDir + '../../js/jscalendar-1.0/calendar-setup.js']);
								loadHTML5Widgets = true;
							}
							break;
						   ...
						   /* yepnope({
							...
							if(!inputSupport.range) {
								toLoad = toLoad.concat([  scriptDir + '../../css/slider.css',
										  scriptDir + '../../js/frequency-decoder.com/slider.js']);
